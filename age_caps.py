# Cosmetic fork of AlienMajik's age plugin (MIT), identical except for the UI
# labels: Age -> AGE, Str -> STR, Pts -> PTS, Next Age -> EXP, Trait -> TRAIT,
# AgeMsg -> MSG.
#
# Two reasons. Uppercase matches the memtemp header, the only other block sharing
# those rows, and both already use DejaVuSansMono-Bold at size 10. And
# LabeledValue places the value at 5 * len(label) pixels while the font renders
# six, so an eight character label like 'Next Age' pushes the value three pixels
# into itself; a three character label leaves a two pixel gap instead.
#
# Renamed from age.py because every configured plugin repository is unpacked into
# the same available-plugins directory, so keeping the upstream filename would
# shadow the original depending on download order.
import os
import json
import logging
import time
...
class Age(plugins.Plugin):
    ...
    __description__ = ('Age plugin with prestige, random events, animated progress, '
                       ...
                       'Cosmetic fork with uppercase labels and EXP in place of Next Age.')
    __help__ = """
    Identical to AlienMajik's age, with uppercase UI labels so they match the memtemp header
    sitting on the same rows. 'Next Age' also becomes 'EXP', which is not only shorter: with an
    eight character label LabeledValue starts the value three pixels before the label ends, so the
    bracket of the progress bar collides with the label. Three characters leave a two pixel gap.

    Disable the upstream 'age' plugin when enabling this one, they would draw on top of each other.

    Layout used on a 250x122 Waveshare 2.13 v3, next to memtemp_caps at 155:

      [main.plugins.age_caps]
      enabled = true
      ...
      strength_x_coord = 66
      strength_y_coord = 96
      age_status_x_coord = 0
      age_status_y_coord = 200

    Elements are hidden by placing them past the bottom of the canvas, which is what upstream does
    with age_status at y 140 on a 122 pixel panel. PIL clips silently.

    Keep the age titles to six characters or fewer: STR starts at x 66, so a longer AGE value runs
    into it.

    Two upstream bugs are also fixed here. Both broke the initial seeding of the handshake count,
    which upstream always left at zero on a jayofelony build:

      - handshake_dir was hardcoded to /root/handshakes. It now follows
        bettercap.handshakes from the pwnagotchi config, and can still be overridden with a
        handshake_dir option.
      - the scan only counted '.pcap' files, while pwnagotchi writes '.pcapng'. Both are counted.
    ...
    def on_loaded(self):
        ...
        # Upstream hardcodes /root/handshakes, which is not where jayofelony builds
        # keep them. Follow bettercap's configured directory, still overridable.
    ...
    def initialize_handshakes(self):
        if self.handshake_count == 0 and os.path.isdir(self.handshake_dir):
            ...
            for root, dirs, files in os.walk(self.handshake_dir):
                # pwnagotchi writes .pcapng; upstream only looked for .pcap, so the
                # seed count was always zero regardless of the directory.
