...
class PiSugar(plugins.Plugin):
    ...
    __description__ = (
        ...
        "Fork with a three state label: BAT on battery, CHG while charging, EXT once full."
    )
    __help__ = """
    Fork of the bundled pisugarx plugin. Two changes.

    The label now has three states instead of two. Upstream only reads bit 7 of register 0x02,
    which means 'external power is present', and labels it CHG, so a fully charged device sitting
    on the charger still reads CHG 100%:
    ...
    There is no 'charge complete' bit anywhere in the PiSugar 3 register map, and the vendor
    admits as much for the LEDs in their FAQ, so fullness has to be inferred from the percentage
    in 0x2A. This fork adds EXT for 'plugged in and done charging':

        BAT 76%    on battery
        CHG 76%    plugged in, below full_level
        EXT 100%   plugged in, at or above full_level

    EXT rather than FULL on purpose: the percentage is right next to it, so a label saying 'full'
    would only repeat it, while EXT tells you something the number does not.

    The threshold is configurable because the percentage is interpolated from a voltage curve and
    wanders around near the top, so an exact 100 would flicker.

      [main.plugins.pisugarx_ext]
      enabled = true
      rotation = false
    ...
    which would have made this fork read the wrong section. It now uses self.options, which the
    plugin loader fills in from whatever section matches the file name.
    ...
    def on_loaded(self):
        ...
        # Upstream reads pwnagotchi.config['main']['plugins']['pisugarx'] by name,
        # which would point at the wrong section now that the file is renamed.
        cfg = self.options
    ...
    def on_ui_update(self, ui):
        ...
        # percentage comes from a voltage curve and drifts around near the top.
        if not battery_plugged:
            ui._state._state['bat'].label = "BAT"
        elif capacity >= self.full_level:
            ui._state._state['bat'].label = "EXT"
        else:
            ui._state._state['bat'].label = "CHG"
